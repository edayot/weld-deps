...
execute if score #already_seen itemio.math matches 1 run return run scoreboard players set #remove_count itemio.math.output 0
...
execute as @e[tag=itemio.transfer.destination.good_distance, limit=1, sort=nearest] at @s run function itemio:v1.5.3/container/output/try_input_after/loop
...
execute if score #success_input itemio.io matches 0 run function itemio:v1.5.3/container/output/try_input_after/add_to_seen_items
